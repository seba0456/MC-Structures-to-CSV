...
public class CheckBlockDestructionProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == MinecraftToCsvModItems.SELECTION_WAND.get()) {
			...
			MinecraftToCsvModVariables.x1 = x;
			MinecraftToCsvModVariables.y1 = y;
			MinecraftToCsvModVariables.z1 = z;
			if (entity instanceof Player _player && !_player.level.isClientSide())
				_player.displayClientMessage(Component.literal(("First edge coordinates: " + ("x:" + x) + (" y:" + y) + (" z:" + z))), false);
		}
	}
}
